    ...
    if(res.documents.length===0){
      ...
      if(cn!==null&&cn!==data.cn){
        if(datas[data.cn]==null){
          if(!window.confirm("You are trying to change the card number to another.This will move the entire card. Do you want to continue?")){
            return null;
          }
          else{
            datas[data.cn]=data;
          ...
          if(!window.confirm("You are trying to change the card number to another.There is already a card present.This will move this card before the specified card. Do you want to continue?")){
            return null;
          }
          else{
            let k=data.cn;
      ...
      if(cn===null){
        if(datas[data.cn]!=null){
            window.alert("Card number already exists. Go to the particular card to edit it.");
            return  null;
        }
        datas[data.cn]=data;
      }
      let ans=datas.filter((card)=>{
        return card!=null;
      }).map((card)=>{
